...
class TrackMatcher:
    """Advanced track matching with configurable algorithms."""
    ...
    def __init__(self, strict_mode: bool = True, enable_duration: bool = True, 
        ...
        if strict_mode:
            self.title_threshold = 0.92
            self.artist_overlap_threshold = 0.50
            self.duration_tolerance = 5  # seconds
        else:
            self.title_threshold = 0.88
            self.artist_overlap_threshold = 0.33
            self.duration_tolerance = 7
    ...
    def _calculate_title_similarity(self, title1: str, title2: str) -> float:
        ...
        # Apply dynamic thresholds based on title length
        word_count = max(len(title1.split()), len(title2.split()))
        if word_count < 3:
            # Stricter for short titles
            threshold = 0.98 if self.strict_mode else 0.95
        else:
            # More lenient for longer titles
            threshold = 0.95 if self.strict_mode else 0.85
    ...
    def _calculate_artist_similarity(self, track1: Track, track2: Track) -> float:
        ...
        return min(1.0, combined_score) if combined_score >= self.artist_overlap_threshold else 0.0
    ...
    def _calculate_duration_similarity(self, duration1: int, duration2: int) -> float:
        ...
        if percentage_diff <= 0.06 if self.strict_mode else 0.10:
            return 1.0
    ...
    def find_best_match(self, target_track: Track, candidate_tracks: List[Track]) -> Optional[Tuple[Track, float]]:
        ...
        for candidate in candidate_tracks:
            ...
            if confidence > best_confidence and confidence >= 0.8:  # Minimum confidence
